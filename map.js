...
// Force map to render correctly (fixes disappearing route)
    ...
    navigator.geolocation.watchPosition(pos => {
        ...
        if (!userMarker) {
            userMarker = L.marker(userLatLng, { 
                icon: L.icon({ iconUrl:'https://maps.gstatic.com/mapfiles/ms2/micons/man.png', iconSize:[32,32] })
...
// Routing with via points for FEU Manila -> FEU Diliman
...
const routeControl = L.Routing.control({
    ...
    createMarker: function(i, wp) {
        if (i === 0) return L.marker(wp.latLng, { 
            icon: L.icon({ iconUrl:'https://maps.gstatic.com/mapfiles/ms2/micons/green.png', iconSize:[32,32] }) 
        });
        if (i === waypoints.length-1) return L.marker(wp.latLng, { 
...
function togglePanel(id) {
    ...
    // Refresh map after panel animation
